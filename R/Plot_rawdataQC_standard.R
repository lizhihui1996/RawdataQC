...
Plot_rawdataQC_standard <- function(data=data,xlab=xlab,ylab=ylab,
  ...
  if (length(data_pass$Var1)==1){
    ...
    {
      SummaryTable_total_reads <- data.frame(Pass=length(data$pass),Fail=0,
  ...
  else
  {
    SummaryTable_total_reads <- data.frame(
      ...
      Total=dim(data)[1],
      Pass_Ratio=round(data_pass[which(data_pass$Var1=="PASS"),
                                 which(colnames(data_pass)=="Freq")]/dim(data)[1],2))
  }
  names(SummaryTable_total_reads) <- c("Number of passed samples","Number of failed samples",
  ...
  p <- p + theme_few()+
    ...
    labs(x=xlab,y=ylab)+
    theme(axis.text.y = element_text(size=axisy_size,face="plain",color = "black"),
  ...
  ### Faillist Plot
  if (dim(SummaryTable_total_reads_faillist)[1]==0) {failplot <- print("There is no fail samples!")}else{
    data_f <- data[data[,which(colnames(data)==ylab)] <  limit,]
    if (showsampletype==T) {failplot <- ggplot(data_f)+
    ...
    failplot <- failplot+scale_fill_nejm(name="Type")+
      ...
      theme(panel.border = element_blank(),
            strip.text.x = element_text(size=20),
            panel.spacing.x = unit(0.2, "mm"))}else{
              failplot <- ggplot(data_f, aes(x=get(xlab), y=get(ylab),fill=pass))+
                geom_bar(stat='identity',alpha=baralpha,width=barwidth)+
                scale_fill_manual(name="Data Quality", values = c("PASS" = bar_pass_color, "FAIL" = bar_fail_color))+
                theme_few()+theme(panel.border = element_blank(),legend.position="none",
    ...
    if(title==T) {failplot <- failplot+ggtitle(paste(titlename,"(Failed Samples)"))}
    if(changelabname==T) {failplot <- failplot+xlab(xlabname) + ylab(ylabname)}
    if(facet==T) {failplot <- failplot+facet_grid(~ get(facet_label), scales = 'free_x', space = 'free_x')}
    if(showerrordetail==T) {failplot <- failplot+geom_text(aes(x=get(xlab), y=max(get(ylab))*1.1,
                                                               label=sprintf(paste0("%.",numberdig,"f"),get(ylab))),
                                                           size=axisy_size*0.5,
                                                           color=errordetail_color,
                                                           position=position_dodge(width=0.9),show.legend = F)}}
  return(list(plot = p, faillist =SummaryTable_total_reads_faillist,
